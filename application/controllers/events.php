...
class Events extends CI_Controller {
	...
	public function index()	{
		if($this->session->userdata('logged_in'))	{
				...
				$config["total_rows"] = $this->pagination_model->count_events();
				$config['per_page'] = 10;				
		        ...
		        $data["results"] = $this->pagination_model->fetch_events($config["per_page"], $page, $data['user']);
	...
	function view($id)	{
		...
		if($this->session->userdata('logged_in'))	{
				...
				//FETCH DATA				
		        $data['items'] = $this->view_model->get_event($id);		      	        
		        ...
		        //PAGINATE ITEMS			            
	   			$config['num_links'] = 5;
				$config['base_url'] = base_url('events/view/'.$id.'/comment/');
			        ...
			        $data['courses'] = $this->alumni_model->course();	
		             ...
		             //ACTION START		
							...
							if ($this->form_validation->run() == FALSE) { 
								...
								if($this->create_model->event($data['id'])){
									...
									$this->session->set_flashdata('success', 'Success! You have published an Event');
		             ...
		             //ACTION START		
							...
							if ($this->form_validation->run() == FALSE) { 
								//ERROR								
									...
									redirect('events/edit/'.$this->input->post('id'), 'refresh');
							}
							else {
								//SUCCESS WITH ANOTHER VALIDATION

								if($this->update_model->event($data['id'])){
									//ULTIMATE SUCCESS
									...
									redirect('events/view/'.$this->input->post('id'), 'refresh');
		...
		if($this->session->userdata('logged_in'))	{  	
		             ...
		             //ACTION START				             			           
						...
						if($this->delete_model->events()){
								    ...
								    //RETURN TO PAGE WITH FLASHDATA. SUCCESS
									$this->session->set_flashdata('success', 'Success! You have deleted an Event');
									redirect('events', 'refresh');
